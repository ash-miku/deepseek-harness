...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). The web profile listens on all interfaces by default and prints LAN URLs; `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence. Use `--host 127.0.0.1` for a loopback-only deployment.
