...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('persists the task completion sound preference and exposes Preview', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-settings-completion-sound'))
    await page.getByRole('button', { name: '设置', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: '设置' })
    await dialog.waitFor({ timeout: 10_000 })
    const selector = dialog.getByRole('button', { name: '叮咚' })
    await selector.waitFor({ timeout: 10_000 })
    expect(await selector.isVisible()).toBe(true)
    await selector.click()
    await page.getByRole('menuitem', { name: '铛' }).click()
    await expect.poll(() => dialog.getByRole('button', { name: '铛' }).isVisible(), { timeout: 5_000 }).toBe(true)
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toContain('completionSoundTone: bell')
    await dialog.getByRole('button', { name: '试听' }).click()
    await dialog.getByRole('button', { name: '铛' }).click()
    await page.getByRole('menuitem', { name: '关闭' }).click()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toContain('completionSound: false')
    await dialog.locator('button[aria-haspopup="menu"]').filter({ hasText: '关闭' }).click()
    await page.getByRole('menuitem', { name: '清脆铃声' }).click()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toContain('completionSoundTone: chime')
    const slider = dialog.getByRole('slider', { name: '提示音音量' })
    await slider.press('ArrowLeft')
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toContain('completionSoundVolume: 75')
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  }, 60_000)
  ...
  it('persists the busy-state Enter behavior across reload and a distinct port', async () => {
    ...
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/ui-conversation:\n(?:\s+(?:completionSound|completionSoundTone|completionSoundVolume): .*\n)*\s+busyEnter: steer/)
    await page.keyboard.press('Escape')
