...
describe.skipIf(process.platform === 'win32')('Web sidebar terminal', () => {
  ...
  it('preserves program palettes and keeps ANSI text and cursors readable across DSH themes', async () => {
    ...
    await expect.poll(() => screen.evaluate(element => getComputedStyle(element).color)).toBe('rgb(26, 28, 31)')
    ...
    await expect.poll(defaults).toEqual({ foreground: 'rgb(255, 255, 255)', background: 'rgb(24, 24, 24)' })
    await selectTerminalTheme(page, 'Light')
    await expect.poll(defaults).toEqual({ foreground: 'rgb(26, 28, 31)', background: 'rgb(255, 255, 255)' })
