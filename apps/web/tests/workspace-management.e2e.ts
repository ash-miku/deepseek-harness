...
describe('web e2e: workspace management (create / rename / grouping / hover affordances)', () => {
  ...
  it('archives the seeded session from its row menu, hiding it durably across reload', async () => {
    ...
    const archivedRow = page.getByRole('treeitem')
      .filter({ has: page.getByText(title, { exact: true }) })
    await expect.poll(() => archivedRow.count(), { timeout: 10_000 }).toBe(1)
    await clickHoverAction(archivedRow.first(), `Session actions for ${title}`)
    await page.getByRole('menuitem', { name: 'Unarchive session' }).click()
    await expect.poll(() => page.getByText(title, { exact: true }).count(), { timeout: 10_000 }).toBe(1)
