...
`host` 接受 `127.0.0.1`（仅回环）和 `0.0.0.0`（所有接口）；Web profile 默认使用 `0.0.0.0`，浏览器信任栅栏会为该绑定采样 LAN IPv4 地址。除该信任栅栏外没有 TLS、认证或 origin 策略，因此绑定所有接口会把服务器暴露给可达网络。dist 位置是认领席位的前端插件的组装事实。
