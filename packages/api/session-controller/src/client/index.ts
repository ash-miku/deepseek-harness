...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
    /**
     * A live durable Turn ended with the Host's successful completed reason.
     * Emitted on the Session's Agent scope, so untagged listeners observe every
     * Session. A reconnect replay of an already-seen seq does not re-emit.
     * @param sessionId - Session whose Turn completed.
     * @param seq - Seq of the closing `turn/end` event.
     * @mode emit
     */
    'session/completed'(sessionId: import('@deepseek-ai/dsh-session/types').SessionId, seq: number): void
  }
}
