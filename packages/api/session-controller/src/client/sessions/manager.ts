...
import { BaselineRetry } from '../baseline-retry.ts'
...
export class SessionManager {
  ...
  /**
   * Backoff driver for the unary list baseline. The list pull is an ordinary
   * Remote call outside the reconnecting stream supervisor, so a transient
   * failure before the first baseline lands would otherwise leave the list at
   * `error` forever.
   */
  private readonly listRetry = new BaselineRetry(() => { void this.refreshList() })
  ...
  refreshList(): Promise<void> {
    // A fresh pull supersedes a queued retry: the caller's attempt replaces it,
    // and leaving the timer armed would pull twice for one failure.
    this.listRetry.cancel()
    ...
    this.listInflight = (async () => {
      try {
        ...
        if (this.listMutations === mutations) {
          ...
          // Only a baseline that never arrived retries on its own; a later
          // pull failure rides the ordinary refresh paths (frames, reconnect,
          // explicit refresh) rather than a background timer.
          if (this.listPhase === 'pending' && this.listState === 'error') this.listRetry.schedule()
          this.notifier.markDirty()
        }
      }
    })()
  ...
  */
  ...
  /**
   * Lose the Host-event generation: stop baseline retries so a dead connection
   * is not hammered on a timer. Opened Session follow streams own their own
   * reconnect through API Gateway and are untouched here.
   */
  handleDisconnected(): void {
    this.listRetry.disconnect()
  }
  ...
  handleConnected(): void {
    // Adopt the generation first: it re-arms retries and restarts the backoff,
    // so this pull's own failure is retried from a fresh delay rather than an
    // inherited one.
    this.listRetry.connect()
