...
describe('list lifecycle', () => {
  ...
  it('retries a failed initial pull while the connection stays active', async ({ mock, remote }) => {
    vi.useFakeTimers()
    let manager: SessionManager | undefined
    try {
      remote.session.list
        .mockResolvedValueOnce(err(new RemoteError('gateway/internal', 'warming up', {})))
        .mockResolvedValueOnce(ok({ items: [summary(S1)] as never[] }))
      manager = makeManager(mock, remote)
      manager.handleConnected()
      await Promise.resolve()
      await Promise.resolve()
      expect(remote.session.list).toHaveBeenCalledTimes(1)
      expect(manager.getListSnapshot().phase).toBe('pending')
      ...
      // A later failure rides the ordinary refresh paths instead: the arrival
      // phase is ready, so no background timer is armed for it.
      remote.session.list.mockResolvedValue(
        err(new RemoteError('gateway/internal', 'later failure', {})),
      )
      await manager.refreshList()
  ...
  it('grows the retry backoff per attempt and stops on connection loss', async ({ mock, remote }) => {
    vi.useFakeTimers()
    try {
      remote.session.list.mockResolvedValue(err(new RemoteError('gateway/internal', 'still down', {})))
      const manager = makeManager(mock, remote)
      manager.handleConnected()
      await Promise.resolve()
      await Promise.resolve()
      ...
      // attempt 0 → 250ms, attempt 1 → 500ms: the shared policy doubles.
      ...
      // Loss stops the timer; a dead connection is not hammered.
      manager.handleDisconnected()
      await vi.advanceTimersByTimeAsync(10_000)
      expect(remote.session.list).toHaveBeenCalledTimes(3)

      // A new generation restarts the budget, so the next retry is the base delay.
      manager.handleConnected()
      await Promise.resolve()
      await Promise.resolve()
      expect(remote.session.list).toHaveBeenCalledTimes(4)
      await vi.advanceTimersByTimeAsync(250)
      expect(remote.session.list).toHaveBeenCalledTimes(5)
    } finally {
      vi.useRealTimers()
    }
  })

  it('never schedules a retry before a connection generation is adopted', async ({ mock, remote }) => {
    vi.useFakeTimers()
    try {
