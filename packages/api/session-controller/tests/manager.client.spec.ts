...
describe('list lifecycle', () => {
  ...
  it('retries a failed initial pull while the connection stays active', async () => {
    ...
    try {
      ...
      api.onList = () => {
        attempts += 1
        return attempts === 1
          ? Promise.resolve(err(new RemoteError('gateway/internal', 'warming up', {})))
          : Promise.resolve(ok({ items: [summary(S1)] as never[] }))
      }
      manager = new SessionManager(fakeRemote(api))
      ...
      api.onList = () => Promise.resolve(err(new RemoteError('gateway/internal', 'later failure', {})))
      await manager.refreshList()
  ...
  it('grows the retry backoff per attempt and stops on connection loss', async () => {
    vi.useFakeTimers()
    try {
      const api = new FakeApiClient()
      api.onList = () => Promise.resolve(err(new RemoteError('gateway/internal', 'still down', {})))
      const manager = new SessionManager(fakeRemote(api))
      manager.handleConnected()
      await Promise.resolve()
      await Promise.resolve()
      ...
      await Promise.resolve()
      await Promise.resolve()
      expect(api.callsOf('session.list')).toHaveLength(4)
      await vi.advanceTimersByTimeAsync(250)
      expect(api.callsOf('session.list')).toHaveLength(5)
    } finally {
      vi.useRealTimers()
    }
  })

  it('never schedules a retry before a connection generation is adopted', async () => {
    vi.useFakeTimers()
    try {
      const api = new FakeApiClient()
