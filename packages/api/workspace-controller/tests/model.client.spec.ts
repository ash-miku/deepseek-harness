...
class FakeWorkspaceRemote implements WorkspaceRemote {
  ...
  onFavoriteSession: (
    request: WorkspaceFavoriteSessionRequest,
  ) => Promise<RemoteResult<WorkspaceFavoriteValue>> = request =>
    Promise.resolve(remoteOk({ favoriteSessionIds: [request.sessionId] }))
  onUnfavoriteSession: (
    request: WorkspaceUnfavoriteSessionRequest,
  ...
  favoriteSession(request: WorkspaceFavoriteSessionRequest): Promise<RemoteResult<WorkspaceFavoriteValue>> {
    this.record('favoriteSession', request)
    return this.onFavoriteSession(request)
  }

  unfavoriteSession(request: WorkspaceUnfavoriteSessionRequest): Promise<RemoteResult<WorkspaceFavoriteValue>> {
...
function baseline(
  ...
  model.replaceBaseline({ items, archivedSessionIds, favoriteSessionIds })
}
...
describe('ClientWorkspaceModel', () => {
  it('replaces reconnect state and applies ordered increments', () => {
    ...
    model.replaceFavorite([sid('pinned')])
    model.removeView(wid('old'))
    expect(model.getSnapshot()).toMatchObject({
      phase: 'ready', state: 'idle', archivedSessionIds: ['hidden'], favoriteSessionIds: ['pinned'],
    })
  ...
  it('installs favorite sets from baselines and increments, preserving favorite order', () => {
    const model = modelFor()
    baseline(model, [], [], [sid('second'), sid('first')])
    expect(model.getSnapshot().favoriteSessionIds).toEqual(['second', 'first'])
    // An identical echo keeps the previous array identity so downstream
    // Object.is short-circuits do not see churn.
    const before = model.getSnapshot().favoriteSessionIds
    model.replaceFavorite([sid('second'), sid('first')])
    expect(model.getSnapshot().favoriteSessionIds).toBe(before)
    model.replaceFavorite([sid('first')])
    expect(model.getSnapshot().favoriteSessionIds).toEqual(['first'])
  })

  it('applies favorite and archive mutation echoes and leaves failures unchanged', async () => {
    const remote = new FakeWorkspaceRemote()
    const model = modelFor(remote)
    baseline(model, [], [sid('archived')], [sid('pinned')])

    remote.onFavoriteSession = () => Promise.resolve(workspaceError(
      new RemoteError('session/not-found', 'missing', { sessionId: sid('missing') }),
    ))
    await expect(model.favoriteSession(sid('missing'))).resolves.toMatchObject({ ok: false })
    expect(model.getSnapshot().favoriteSessionIds).toEqual(['pinned'])

    remote.onFavoriteSession = request => Promise.resolve(remoteOk({
      favoriteSessionIds: [request.sessionId, sid('pinned')],
    }))
    await expect(model.favoriteSession(sid('fresh'))).resolves.toMatchObject({ ok: true })
    expect(remote.calls).toContainEqual({ method: 'favoriteSession', request: { sessionId: 'fresh' } })
    expect(model.getSnapshot().favoriteSessionIds).toEqual(['fresh', 'pinned'])

    remote.onUnfavoriteSession = () => Promise.resolve(remoteOk({ favoriteSessionIds: [] }))
    await expect(model.unfavoriteSession(sid('pinned'))).resolves.toMatchObject({ ok: true })
    expect(remote.calls).toContainEqual({ method: 'unfavoriteSession', request: { sessionId: 'pinned' } })
    expect(model.getSnapshot().favoriteSessionIds).toEqual([])

    remote.onUnarchiveSession = request => Promise.resolve(workspaceError(
      new RemoteError('session/not-found', 'missing', { sessionId: request.sessionId }),
    ))
    await expect(model.unarchiveSession(sid('archived'))).resolves.toMatchObject({ ok: false })
    ...
    await expect(model.unarchiveSession(sid('archived'))).resolves.toMatchObject({ ok: true })
    expect(remote.calls).toContainEqual({ method: 'unarchiveSession', request: { sessionId: 'archived' } })
    expect(model.getSnapshot().archivedSessionIds).toEqual([])
  })
