...
class ScriptedWorkspaceRemote implements WorkspaceRemote {
  ...
  favoriteSession(_request: WorkspaceFavoriteSessionRequest): Promise<RemoteResult<WorkspaceFavoriteValue>> {
    throw new Error('unused')
  }

  unfavoriteSession(_request: WorkspaceUnfavoriteSessionRequest): Promise<RemoteResult<WorkspaceFavoriteValue>> {
    throw new Error('unused')
  }
...
class CommandWorkspaceRemote implements WorkspaceRemote {
  ...
  readonly favoriteSession = vi.fn<WorkspaceRemote['favoriteSession']>(request => Promise.resolve(remoteOk({
    favoriteSessionIds: [request.sessionId],
  })))

  readonly unfavoriteSession = vi.fn<WorkspaceRemote['unfavoriteSession']>(() => Promise.resolve(remoteOk({
    favoriteSessionIds: [],
  })))
  ...
  it('delivers one baseline followed by increments', async () => {
    ...
    const remote = new ScriptedWorkspaceRemote([{
      frames: [
        ...
        { type: 'favorite', favoriteSessionIds: ['session-two' as never] },
      ],
      hold: true,
    }])
    ...
    const replaceFavorite = vi.fn<WorkspaceFollowSink['replaceFavorite']>()
    ...
    stream.start()
    stream.start()
    await vi.waitFor(() => { expect(replaceFavorite).toHaveBeenCalledOnce() })
    ...
    expect(replaceFavorite).toHaveBeenCalledWith(['session-two'])
...
describe('WorkspaceController', () => {
  it('publishes the model source and exposes successful Workspace commands', async () => {
    ...
    model.replaceBaseline({ items: [workspace('one')], archivedSessionIds: [], favoriteSessionIds: [] })
  ...
  it('maps generated business failures to the command facade errors', async () => {
    ...
    remote.favoriteSession.mockResolvedValueOnce(remoteFailure(missingSession))
    ...
    remote.unfavoriteSession.mockResolvedValueOnce(remoteFailure(missingSession))
