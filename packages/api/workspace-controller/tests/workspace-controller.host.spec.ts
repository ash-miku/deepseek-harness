...
describe('WorkspaceController commands', () => {
  ...
  it('reorders Workspaces and Sessions and archives only known Sessions', async () => {
    ...
    // An id outside the archive set is an idempotent no-op, never a failure.
    await expect(controller.unarchiveSession({ sessionId: SessionId('never-archived') }))
      .resolves.toEqual({ archivedSessionIds: [] })
  })

  it('favorites and unfavorites only known Sessions', async () => {
    const { controller, ctx, root } = await harness()
    const created = await controller.create({ path: stageDir(root, 'pins') })
    const session = ctx.sessions.create(SessionId('pinned'), {
    ...
    await expect(controller.favoriteSession({ sessionId: SessionId('unknown') }))
      .rejects.toMatchObject({ code: 'session/not-found' })
    // Favoriting twice is idempotent — the set keeps one entry.
    await expect(controller.favoriteSession({ sessionId: session.id }))
      .resolves.toEqual({ favoriteSessionIds: [session.id] })
    ...
    await expect(controller.unfavoriteSession({ sessionId: session.id }))
      .resolves.toEqual({ favoriteSessionIds: [] })
    expect(ctx.workspaceRegistry.favoriteSessionIds).toEqual([])
  })
})
...
describe('WorkspaceController follow', () => {
  ...
  it('starts with a complete baseline and emits committed increments in domain order', async () => {
    ...
    await expect(nextFrame(iterator)).resolves.toEqual({
      type: 'baseline',
      value: { items: [], archivedSessionIds: [], favoriteSessionIds: [] },
    })
    ...
    await expect(nextFrame(iterator)).resolves.toEqual({
      type: 'favorite', favoriteSessionIds: [session.id],
    })
    ...
    await expect(nextFrame(iterator)).resolves.toEqual({
      type: 'favorite', favoriteSessionIds: [],
    })
