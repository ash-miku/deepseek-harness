...
A target becomes active when shell selection resolves it or when its source receives a first subscriber. The assembler replaces that target from current Contexts once and keeps it active for later incremental flushes; creating a source does not activate it and unsubscription does not deactivate it. The General settings page includes a Host-backed completion-sound preference with Off, Ding-dong, Bell, and Bright chime choices plus an application-level volume slider. The runtime emits a notification only for a structured turn/end whose reason is completed; pointer or keyboard interaction unlocks the Web Audio context so automatic playback stays within browser policy, and Preview plays the selected tone on demand.
