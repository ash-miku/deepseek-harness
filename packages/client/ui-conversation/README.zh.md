...
shell 选择解析出 target 或 target source 收到首个 subscriber 时，该 target 进入 active 状态。assembler 从当前 Context 对它执行一次 replace，并使它参与后续增量 flush；创建 source 不会激活 target，取消订阅也不会停用 target。通用设置页包含由 Host 支撑的任务完成提示音偏好，提供「关闭」「叮咚」「铛」「清脆铃声」和应用内音量滑块。runtime 只会在结构化 `turn/end` 的 reason 为 `completed` 时发出提示；首次点击或按键会解锁 Web Audio 上下文，试听操作则按需播放当前音色，使自动播放符合浏览器策略。
