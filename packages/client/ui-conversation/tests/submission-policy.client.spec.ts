...
describe('ComposerSubmissionPolicy', () => {
  ...
  it('adopts a Host preference without writing it back and leaves an identical write untouched', () => {
    ...
    host.publish({ status: 'ready', value: { busyEnter: 'steer', processDisplay: 'full', runningLabels: DEFAULT_RUNNING_LABELS, completionSound: DEFAULT_COMPLETION_SOUND, completionSoundTone: DEFAULT_COMPLETION_SOUND_TONE, completionSoundVolume: DEFAULT_COMPLETION_SOUND_VOLUME }, revision: 1, writable: true })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
    policy.setBusyEnter('steer')
    expect(host.set).not.toHaveBeenCalled()
    host.publish({ value: { busyEnter: 'steer', processDisplay: 'full', runningLabels: DEFAULT_RUNNING_LABELS, completionSound: DEFAULT_COMPLETION_SOUND, completionSoundTone: DEFAULT_COMPLETION_SOUND_TONE, completionSoundVolume: DEFAULT_COMPLETION_SOUND_VOLUME }, revision: 2 })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
  })
