...
type LayoutActions = {
  ...
  /**
   * Geometry-free sidebar restatement kept from the custom branch: the mobile
   * drawer close. It closes the narrow-expanded override and is a no-op while
   * wide, so desktop selection flows keep the sidebar open.
   */
...
export function createLayoutStore(): EngineStoreHandle<LayoutState, LayoutActions>  {
  const handle = defineStore({
    ...
    actions: {
      ...
      // The breakpoint reading is derived here (this store owns viewportWidth)
      // rather than mirrored into separate state.
      collapseSidebar: (d) => {
        if (d.layoutInfo.viewportWidth < SIDEBAR_AUTO_COLLAPSE) d.layoutInfo.narrowExpanded = false
      },
