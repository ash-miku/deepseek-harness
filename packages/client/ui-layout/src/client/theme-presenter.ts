...
/** Body variable carrying the user's interface base font weight. */
export const CONTENT_FONT_WEIGHT_VARIABLE = '--dsh-content-font-weight'
...
export class ThemePresenter {
  ...
  apply(snapshot: ThemeSnapshot): void {
    ...
    body.style.setProperty(CONTENT_FONT_WEIGHT_VARIABLE, String(snapshot.fontWeight))
  ...
  dispose(): void {
    ...
    body.style.removeProperty(CONTENT_FONT_WEIGHT_VARIABLE)
    for (const name of this.appliedTokens) body.style.removeProperty(name)
    this.appliedTokens = []
