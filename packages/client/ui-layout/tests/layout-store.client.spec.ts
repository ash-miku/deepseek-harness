...
describe('createLayoutStore', () => {
  ...
  it('collapseSidebar closes the narrow drawer and keeps the width preference', () => {
    const { store, actions } = createLayoutStore().create()
    actions.setSidebar(400)
    actions.setViewportWidth(980)
    actions.toggleSidebar()
...
describe('main panel selection', () => {
  ...
  it.each(['setSidebar', 'toggleSidebar', 'setViewportWidth', 'setRightbar', 'openRightbar', 'closeRightbar', 'collapseSidebar'] as const)(
    'preserves panelInfo identity when %s changes layoutInfo', (action) => {
      const { store, actions } = createLayoutStore().create()
      actions.selectPanel(panelA)
      if (action === 'closeRightbar') actions.openRightbar(true, true)
      // collapseSidebar only writes while narrow, so drive it into the drawer state.
      if (action === 'collapseSidebar') { actions.setViewportWidth(980); actions.toggleSidebar() }
