...
Mount this plugin alongside `ui-conversation` and `dsh-plan-mode`; the chip then occupies the composer's plan seat to the right of the access-mode control. Enter plan mode through the `/plan` command path — choose Plan from the composer's `+` Command menu, type `/plan`, or use the seat's visible Plan control — and turn it off with the chip.

### What the chip shows

The seat renders a quiet "Plan" entry while the host-computed `plan` projection's effective target is inactive, and the warn-colored "Plan ×" status button while it is active; the effective target is `pending ? !active : active` — a folded host value, not client optimism, so an arriving frame corrects the chip either way. Clicking the control executes `/plan` or `/plan off` through `command.execute`. A host without plan mode, or a Draft with no session, shows nothing. While plan mode is the effective target, the composer textarea's placeholder switches to the plan-task hint — "describe your task to generate plan", localized through ui-conversation's `conversation` locale namespace (the `placeholder.plan` / `hint.plan` keys), shared verbatim with the claimed `/plan` command hint (rendered by the composer from the same projection) — unless the owning surface supplies its own placeholder.
...
- **The inactive Plan control is quiet, not absent** — a session with the capability but inactive mode shows the same seat entry, rendered without the warn treatment.
