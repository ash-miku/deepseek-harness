...
describe('PlanChip', () => {
  ...
  it('renders an inactive Plan entry and enters plan mode on click', async () => {
    ...
    await waitFor(() => {
      expect((screen.getByRole('button', { name: 'plan mode 已开启，按下关闭' }) as HTMLButtonElement).disabled).toBe(false)
    })
  })
  ...
  it('shows an enter-mode failure on the inactive control', async () => {
    ...
    fireEvent.click(inactiveChip())
    expect(await screen.findByText('failed to enter plan mode')).toBeTruthy()
