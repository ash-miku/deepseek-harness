...
describe('ui-settings-general apply', () => {
  ...
  it('seats the Host document action on an off-loopback page once the trust fence accepts it', async ({ mock, start }) => {
    ...
    // Local-DSH custom: a LAN/ZeroTier page reaches the same Host surface once
    // its first generation clears the /api trust fence, so the document action
    // is seated and its controller reads the real mirror rather than an
    // off-loopback process-local copy.
    expect(ownEntries(c, 'settings.action').map(entry => entry.component)).toEqual([SettingsDocumentAction])
    expect(actionInjectedOf(c).controller.store.getSnapshot().status).toBe('idle')
    await c.unload(SELF)
    await c.flush()
