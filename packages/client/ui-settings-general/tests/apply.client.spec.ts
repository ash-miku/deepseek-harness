...
async function bench(isLoopback = true) {
  ...
  ctx.provide('connection', {
    ...
    reconnect: () => {},
    // Loopback pages register the document action immediately; non-loopback
    // pages wait on the first Host generation, so the stub reports one.
    generation: { getSnapshot: () => (isLoopback ? {} : undefined), subscribe: () => () => {} },
  } as never)
