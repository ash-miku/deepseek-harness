...
describe('WelcomeNoticeStore', () => {
  ...
  it('downgrades a refused probe page to memory instead of surfacing the fence error', async () => {
    ...
    const { mirror, controller } = buildWelcome({ describe, mutate }, 'probe')
  ...
  it('persists a fence-accepted probe page through Host settings', async () => {
    ...
    const mutate = vi.fn(() => Promise.resolve(ok(acknowledgedNamespace(WELCOME_NOTICE_VERSION))))
    const { mirror, controller } = buildWelcome({ describe, mutate }, 'probe')
    ...
    await expect(controller.acknowledge()).resolves.toBe(true)
    // The wire contract is positional (settings/mutate: ns, ops, expectedRevision).
