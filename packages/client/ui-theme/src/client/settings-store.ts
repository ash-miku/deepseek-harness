...
type AppearanceRowActions = {
  sync: (draft: AppearanceRowState, preference: ThemePreference, fontScale: FontScalePreference, revision: number) => void
}
...
export function createAppearanceRowStore(): EngineStoreHandle<AppearanceRowState, AppearanceRowActions> {
  return defineStore({
    init: (): AppearanceRowState => ({ preference: 'system', fontScale: 'normal', revision: -1 }),
    actions: {
      sync: (d, preference: ThemePreference, fontScale: FontScalePreference, revision: number) => {
