...
import type { ThemeFontWeight, ThemePreference } from './theme-settings.ts'
...
import {
  DEFAULT_FONT_SIZE, DEFAULT_FONT_WEIGHT, DEFAULT_PREFERENCE, FONT_SIZE_MIN, FONT_SIZE_MAX, FONT_WEIGHTS, THEME_PREFERENCES,
} from './theme-settings.ts'

export {
  DEFAULT_FONT_SIZE, DEFAULT_FONT_WEIGHT, DEFAULT_PREFERENCE, FONT_SIZE_FIELD, FONT_SIZE_MAX, FONT_SIZE_MIN,
  FONT_WEIGHT_FIELD, FONT_WEIGHTS, THEME_PREFERENCE_FIELD, THEME_PREFERENCES, THEME_SETTINGS_NAMESPACE,
...
export const Config = z.object({
  ...
  fontWeight: z.union([...FONT_WEIGHTS]).default(DEFAULT_FONT_WEIGHT).volatile(),
})
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.on('webserver/index-inject', (table) => {
    table.push(...bootThemeInjections(config.preference.get(), config.fontSize.get(), config.fontWeight.get()))
  }, { prepend: true })
}
