...
async function bench(isLoopback = true) {
  ...
  // A non-loopback page probes Host persistence and may be refused at the
  // trust fence; model that refusal so the probe path is exercised.
  if (!isLoopback) {
...
describe('ui-theme apply', () => {
  ...
  it('loads Host settings at boot, refreshes its namespace, and keeps remote browsers process-local', async () => {
    ...
    remote.ctx.emit('connection/reset')
