...
import {
  DEFAULT_FONT_SCALE, DEFAULT_PREFERENCE, THEME_SETTINGS_NAMESPACE, apply,
...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    await ctx.settings.update(ns, { preference: 'dark', fontScale: 'large' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', fontScale: 'large' })
    await expect(ctx.settings.update(ns, { preference: 'sepia' })).rejects.toThrow()
    await expect(ctx.settings.update(ns, { fontScale: 'huge' })).rejects.toThrow()
    await fiber.dispose()
