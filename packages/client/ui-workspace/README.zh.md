...
批量归档操作位于视图和添加操作旁。它打开一个支持 7、30、90、180 天阈值的对话框，先预览符合条件的数量，再对每个候选调用同一个幂等 `archiveSession` 操作。当前、运行中、待交互、空白、子代理和已归档会话会被排除；成功、部分失败和没有候选的结果都会留在对话框中。
...
- **没有 Session 删除控件**：已归档会话可以查看和恢复，但没有 Session 删除入口；删除 Workspace 注册记录不会删除 Session。
