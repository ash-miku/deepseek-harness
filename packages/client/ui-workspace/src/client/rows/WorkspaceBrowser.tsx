 ...
 * region icons (search / bulk-archive / add workspace) as 36px controls on the
 * shell's shared rail entry path, each requesting expansion through the owner
 * share. Adding
...
> & {
  ...
  /** Registry-global archive set (row flags and the trailing bucket). */
...
function SessionTree({
  useSessions, useSessionPendingInteraction, startSession, open, forkSession, workspaces,
  ...
  const groups = useMemo(
    ...
    [
      ...
      expandedGroups, sessionOrderByAccount, favoriteSessionIds,
    ],
  )
  useEffect(() => {
...
function FlatList({
  ...
  onSessionUnarchive, onSessionFavorite, onSessionUnfavorite,
  archivedSessionIds, favoriteSessionIds, usePanelInfo,
...
>) {
  ...
  const baseRows = useMemo(
    () => deriveFlat(list, archivedSessionIds, pendingInteractions, favoriteSessionIds),
    [list, archivedSessionIds, pendingInteractions, favoriteSessionIds],
  )
...
export function WorkspaceBrowser({
  ...
  // Unarchive is dialog-free for the same reason as archive: it only changes
  // grouping visibility, never the log or the workspace accounting slot.
  ...
  // Pinning is likewise visibility-only: the row moves to the leading bucket
  // and its Workspace accounting is untouched.
