...
describe('WorkspaceBrowser', () => {
  ...
  it('bulk archives only inactive idle sessions through the header dialog', async () => {
    ...
    mount({
      ...
      // Pending state is not a list field any more: it arrives on the
      // Session-owned interaction snapshot, so the eligible set must drop it.
      useSessionPendingInteraction: ((selector: (state: SessionPendingInteractionSnapshot) => unknown) =>
        selector(new Map([[sid('pending'), { key: 'approval:1', kind: 'approval', sessionId: sid('pending') }]] as unknown as SessionPendingInteractionSnapshot))) as WorkspaceBrowserProps['useSessionPendingInteraction'],
      archiveSession,
    })
