...
describe('WorkspaceBrowser', () => {
  ...
  it('bulk archives only inactive idle sessions through the header dialog', async () => {
    ...
    mount({
      ...
      useSessionStatus: ((selector: (state: SessionStatusSnapshot) => unknown) =>
        selector(new Map<SessionId, SessionStatus>([[sid('pending'), {
          running: undefined,
          pendingInteraction: { key: 'approval:1', kind: 'approval', sessionId: sid('pending') } as SessionPendingInteraction,
          completionUnread: false,
        }]]))) as WorkspaceBrowserProps['useSessionStatus'],
      archiveSession,
    })
