...
import { deepseekBalanceValueSchema } from '../api/deepseek.schema.ts'
import {
...
export interface IApiClient {
  ...
  deepseek: {
    balance(payload: RequestPayload<'deepseek.balance'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'deepseek.balance'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'deepseek.balance': deepseekBalanceValueSchema,
}
...
function browserSafeRandomUuid(): string {
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    return RpcId(browserSafeRandomUuid())
  }
  ...
  readonly deepseek: IApiClient['deepseek'] = {
    balance: (payload, signal) => this.callUnary('deepseek.balance', payload, signal),
  }
