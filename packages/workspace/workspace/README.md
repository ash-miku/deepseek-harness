...
- `ctx.workspaceRegistry.archiveSession(id)` / `unarchiveSession(id)` / `archivedSessionIds` — the registry-global archive set, layered over workspace accounting: an archived session disappears from ordinary grouping surfaces but keeps its session log and its `sessionIds` slot, so `unarchiveSession` restores its position. Archiving accepts any live or persisted session (accounted or Ungrouped), resolves without writing for an already archived id, and rejects an unknown id; unarchiving an id outside the set is an idempotent no-op. State written before the field existed parses with an empty set.
